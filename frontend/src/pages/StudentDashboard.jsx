...
export default function StudentDashboard({ courseId }) {
  ...
  return (
    // make the whole page fill the viewport height and width
    <div className="min-h-screen h-screen w-full bg-fixed bg-cover bg-center" style={{ backgroundImage: 'linear-gradient(180deg,#f0f4ff 0%, #fff0f6 100%)' }}>
      {/* Full-width fixed header with new background (solid dark gradient) */}
        ...
        <div className="max-w-7xl mx-auto px-8 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-4">
              ...
              <button onClick={logout} aria-label="Logout" className="inline-flex items-center gap-2 px-3 py-2 bg-white/10 hover:bg-white/20 rounded-md transition">
                <FaSignOutAlt />
            ...
            <div className="sm:hidden flex items-center">
              <button onClick={logout} aria-label="Logout" className="p-2 rounded-lg bg-white/10">
                <FaSignOutAlt />
              </button>
            </div>
          </div>
      ...
      {/* content area below header — use full height and offset for fixed header */}
      <main className="pt-28 px-6 h-full overflow-auto">
        {/* decorative blurred blobs fixed to viewport */}
        <div className="pointer-events-none fixed -left-40 -top-40 w-96 h-96 rounded-full bg-gradient-to-tr from-pink-300 via-indigo-300 to-purple-400 opacity-30 blur-3xl transform -rotate-12" />
        <div className="pointer-events-none fixed -right-40 -bottom-40 w-96 h-96 rounded-full bg-gradient-to-br from-amber-200 via-orange-200 to-pink-200 opacity-25 blur-3xl transform rotate-6" />

        <div className="max-w-6xl mx-auto relative z-10">
          {/* Header card */}
          <div className="bg-white/80 backdrop-blur-md border border-white/40 rounded-3xl shadow-2xl p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center gap-6 animate-fadeInDown">
            <div className="flex items-center gap-4">
              <div className="bg-gradient-to-br from-orange-500 to-purple-400 text-white rounded-xl p-4 shadow-lg">
                <FaBook className="w-6 h-6" />
              </div>
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Welcome back, {user?.name || "Student"} 🎓</h1>
                <p className="text-sm text-gray-500 mt-1">Continue your learning — pick a quiz or explore courses.</p>
              </div>
            </div>

            <div className="ml-auto w-full md:w-auto flex flex-col md:flex-row gap-3">
              <Link to="/course1" className="flex items-center justify-center gap-3 px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg font-semibold shadow-lg transform hover:-translate-y-0.5 transition" aria-label="Browse courses">
              ...
              {user?.role === "instructor" && (
                <button type="button" onClick={handleCreateQuiz} className="flex items-center justify-center gap-2 px-4 py-2 bg-white border border-orange-200 text-orange-600 rounded-lg font-semibold hover:shadow-md transition">
                  <FaPlus /> Create Quiz
                </button>
              )}
            </div>
          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm p-6 animate-fadeInUp">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-sm text-gray-500">{loading ? "Loading..." : `${debugInfo?.returnedCount ?? quizzes.length} items`}</div>
              </div>
            ...
            <aside className="bg-white rounded-2xl shadow-sm p-6 space-y-4 animate-fadeInRight">
              ...
              <Link to="/course1" className="w-full mt-2 flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg font-semibold shadow-lg hover:-translate-y-0.5 transition">
